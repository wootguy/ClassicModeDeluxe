// TODO: 
// rpg grunt (remove rocket from model)
// grapple (fix tongue position)
// fix minigun anims
// saw muzzle flash in wrong spot
// disable friendly otis/barney until there are proper models?
// body guard broken anims
...
namespace AutoClassicMode {
	...
	{
		isClassicMap = caller.pev.rendermode == 1 and !forceDisable;
		...
		for (uint i = 0; i < MAX_ITEM_TYPES; i++)
		{
			...
			while (item !is null)
			{
				...
				if (wep !is null)	
				{
					...
					{
						plr.pev.viewmodel = GetWeaponVModel(wep);
						plr.pev.weaponmodel = GetWeaponPModel(wep);
					}
				}
	...
	void MonsterKilled(CBaseEntity@ activator, CBaseEntity@ caller, USE_TYPE useType, float value)
	{
		...
		g_Scheduler.SetTimeout("UpdateMonsterModels", 0.15f);
		g_Scheduler.SetTimeout("UpdateModels", 0.15f, "weapon_sniperrifle");
	}
	...
	void UpdateMonsterModels()
	{
		...
		do {
			...
			if (mon !is null)
			{
				println("Test " + mon.pev.model);
				if (modelReplacements.exists(mon.pev.model))
				{
					println("Le model replace " + ent.pev.classname);
					string replacement = GetReplacementModel(mon.pev.model);
					
					/*
					if (ent.pev.classname == "monster_human_grunt" and ent.pev.weapons & 64 != 0) // rpg grunt
						mon.pev.body = g_ModelFuncs.SetBodygroup( g_ModelFuncs.ModelIndex("models/AutoClassicMode/hgrunt.mdl"), mon.pev.body, 2, 3);
					if (int(string(ent.pev.classname).Find("robogrunt")) != -1 and ent.pev.weapons & 8 != 0) // robo grunt with shotgun
						mon.pev.body = g_ModelFuncs.SetBodygroup( g_ModelFuncs.ModelIndex("models/AutoClassicMode/rgrunt.mdl"), mon.pev.body, 2, 1);
					*/
					...
					if (int(cname.Find("grunt")) != -1 or cname == "monster_male_assassin" or cname == "monster_assassin_repel" or cname == "monster_bodyguard")
					{
						if (ShouldUpdateSoundlist(mon))
						{
							println("Add soundlist to " + ent.pev.classname);
							ent.KeyValue("soundlist", "../AutoClassicMode/weapons.txt");
	...
	{		
		...
		do {
			...
			{
				string cname = ent.pev.classname;
				if (cname == "monster_hwgrunt")
				{
					MonitorHWGrunt(EHandle(ent));
					continue;
				}
					
				bool dropsWeps = cname == "monster_shocktrooper";
				if (!dropsWeps and cname == "monster_male_assassin")
					dropsWeps = ent.pev.weapons == 8 or ent.pev.weapons == 10; // sniper, sniper + HG
				if (!dropsWeps)
					continue;
				
				string target = string(mon.m_iszTriggerTarget).ToLowercase();
				if (mon.m_iszTriggerTarget == spawnHookName)
					continue;
				if (target.Length() > 0)
				{
					if (mon.m_iTriggerCondition == 4)
						...
						println("Failed to add death hook for " + ent.pev.classname);
				}
				else
				{
					ent.pev.target = spawnHookName;
